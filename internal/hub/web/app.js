...
function renderMachines(agents) {
    const cards = agents.map((agent) => {
        ...
        values.forEach(([label, value], index) => {
            ...
            if (index === 3 && agent.publicCount)
                row.append(element("span", "warn", ` · ${agent.publicCount} 公网`));
            stats.append(row);
        });
        card.append(stats);
...
function renderServices(groups) {
    ...
    groups.forEach((group) => {
        ...
        if (!group.agent.online)
            title.append(element("span", "err", " · 离线"));
        sections.push(title);
        const container = element("div", "card section-card");
        const services = [...group.services].sort((left, right) => exposureOrder[serviceExposure(left)] - exposureOrder[serviceExposure(right)]);
        if (!services.length) {
...
function serviceRow(agentID, service) {
    ...
    if (service.unidentified)
        name.append(element("span", "err", " ?"));
        ...
        serviceRoutes.forEach((route) => {
            ...
            if (routeHref) {
                ...
                if ((route.upstreams ?? []).length)
                    link.title = `上游：${route.upstreams?.join(", ")}`;
                routes.append(link);
            }
            else {
