...
- Nginx 采集器现在能识别具名虚拟主机直接提供的静态内容，但不输出文件系统根路径；服务页可搜索并展开全部域名、路径、类型和无凭据上游关系。
- 脱敏回归样例覆盖受保护静态站点、刷新代理和返回 502 的废弃域名候选。资料编辑与未来受控退役操作在产品语义上分离。

## [2026-08-08] product | Operations 发布终态与主机审计范围

- 真实 canary 操作反馈表明，旧页面在“发布已受理”后只把最终状态写入顶部提示和页面末端审计，操作者容易无法判断成功、失败或自动回滚。
- 发布弹窗现在会明确经历“发布进行中”并在终态直接显示发布成功、回滚完成、自动回滚或失败；页面顶部仍保留简明结果提示。
